  ...
  run: async (client, message, args) => {
    Guild.findOne({
      ...
      message.channel
        ...
        .then(async () => {
          message.channel.send(`${user} remove from ticket ${message.channel}`);
          let log = guild.log
          ...
          if (echannel && message.guild.me.permissionsIn(echannel).has("VIEW_CHANNEL") && message.guild.me.permissionsIn(echannel).has("EMBED_LINKS") && message.guild.me.permissionsIn(echannel).has("SEND_MESSAGES")) {
            try {
              ...
              let embed = new Discord.MessageEmbed()
                .setTitle(`New user removed from ticket`)
                .setDescription(`The user (${user}) has been removed from ticket <#${message.channel.id}> by ${message.author}`)
